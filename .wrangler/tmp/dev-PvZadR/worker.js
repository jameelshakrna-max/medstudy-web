...
__name(handleGetResources, "handleGetResources");
...
__name(handleGetResource, "handleGetResource");
function extractResourceIdFromPath(url) {
  const parts = new URL(url).pathname.split("/");
  return parts[3];
}
__name(extractResourceIdFromPath, "extractResourceIdFromPath");
async function handleCreateResource(request, env, user) {
  ...
  await env.DB.prepare(
    `INSERT INTO resources (id, title, category, description, tags, file_name, file_key, file_size, mime_type, image_key, user_id, user_name, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, title.trim(), category, description, tags, file.name, fileKey, file.size, file.type || "", imageKey, user.sub, user.email?.split("@")[0] || "User", now).run();
  const { results } = await env.DB.prepare("SELECT * FROM resources WHERE id = ?").bind(id).all();
  return json(mapResource(results[0]), 201);
}
__name(handleCreateResource, "handleCreateResource");
...
__name(handleUpdateResource, "handleUpdateResource");
...
__name(handleDeleteResource, "handleDeleteResource");
...
__name(handleGetResourceFile, "handleGetResourceFile");
...
__name(handleGetResourceImage, "handleGetResourceImage");
...
__name(handleDownloadResourceFile, "handleDownloadResourceFile");
...
async function handleCreateComment(request, env, user) {
  ...
  ).bind(id, resourceId, body.parent_id || null, user.sub, user.email?.split("@")[0] || "User", body.content.trim()).run();
...
__name(handleVoteComment, "handleVoteComment");
...
__name(mapResource, "mapResource");
function mapCard(r) {
  return {
    id: r.id,
    user_id: r.user_id,
...
// .wrangler/tmp/bundle-SzAGUU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SzAGUU/middleware-loader.entry.ts
