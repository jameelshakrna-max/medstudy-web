...
export async function GET(req) {
  ...
  try {
    const [deckResult, cardResult] = await Promise.all([
      turso.execute({ sql: 'SELECT * FROM anki_decks WHERE user_id = ? ORDER BY name ASC', args: [user.id] }),
      turso.execute({ sql: "SELECT * FROM anki_cards WHERE user_id = ? ORDER BY CASE WHEN next_review IS NULL AND next_review_date IS NULL THEN 1 ELSE 0 END, COALESCE(next_review, next_review_date) ASC, created_at DESC", args: [user.id] })
    ])
