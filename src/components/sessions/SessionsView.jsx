...
const StatsCards = memo(function StatsCards({ totalMin, totalSessions, avgDuration, goalsMet }) {
  return (
    <div className={styles.statsGrid} data-testid="session-stats">
      <div className={`${styles.statCard} ${styles.statBlue}`}>
        <div className={styles.statValue}>{(totalMin / 60).toFixed(1)}</div>
      ...
      <div className={`${styles.statCard} ${styles.statBrand}`}>
        <div className={styles.statValue}>{avgDuration}</div>
        <div className={styles.statLabel}>Avg Min</div>
      </div>
      <div className={`${styles.statCard} ${styles.statAmber}`}>
        <div className={styles.statValue}>{goalsMet}</div>
        <div className={styles.statLabel}>Goals Met</div>
      </div>
    </div>
  )
})
...
export default function SessionsView() {
  ...
  const { data: sessions = [], isLoading, isError, isRefetchError, refetch } = useQuery({
    queryKey: queryKeys.sessions.list(user?.id),
    queryFn: () => supabase.from('study_sessions').select('*').eq('user_id', user.id).order('date', { ascending: false }).order('created_at', { ascending: false }).limit(200).then(d => {
  ...
  const energyClass = e => e === 'High' ? 'energyHigh' : e === 'Medium' ? 'energyMedium' : 'energyLow'
  ...
  if (isError && (sessions?.length || 0) === 0) {
    return <QueryErrorState message="Couldn't load your study sessions." onRetry={() => refetch()} />
  }

  return (
    <div className={styles.root} data-testid="sessions-view">
      {isRefetchError && (sessions?.length || 0) > 0 && <RefetchWarning onRetry={() => refetch()} />}
      ...
      <div className={styles.header}>
        ...
        <p className={styles.sub}>
          ...
          {streak > 1 && <span className={styles.streak}><Flame size={14} strokeWidth={1.5} /> {streak} day streak</span>}
        </p>
        <p className={styles.sub}>Track your study time — manual entries and completed Focus sessions both appear here.</p>
      </div>
      ...
      {view === 'add' ? (
        ...
        <>
          ...
          {sessionTypes.length > 2 && (
            <div className={`${styles.filterRow} ${styles.typeFilterRow}`}>
              {sessionTypes.map(t => (
          ...
          <div className={styles.cardList} data-testid="session-list">
                ...
                {items.map(s => (
                  <div key={s.id} className={styles.sessionCard}>
                    ...
                    <div className={styles.sessMeta}>
                      ...
                      <span className={styles[energyClass(s.energy_level)]}>Energy: {s.energy_level}</span>
                      <span>{s.focus_quality}</span>
                      {s.goals_met && <span className={styles.goalsMet}>Goals Met</span>}
                    </div>
                    {s.notes && <div className={styles.sessNotes}>{s.notes}</div>}
                    <button className={styles.sessDelete} onClick={() => deleteSession(s.id)} title="Delete" aria-label="Delete session"><X size={12} strokeWidth={2} /></button>
                  </div>
                ))}
              </div>
            ))}
