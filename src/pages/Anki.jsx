...
function isDue(c) { if (!c.last_review) return true; if (!c.next_review) return true; return new Date(c.next_review) <= new Date() }
function isNew(c) { return !c.last_review }
function statusInfo(c) { if (!c.last_review) return { l: 'New' }; if (isDue(c)) return { l: 'Due' }; return { l: 'Later' } }
function nextReviewLabel(c) {
  if (!c.last_review) return { t: 'Not scheduled' }
  if (!c.next_review) return { t: 'Due now' }
  const now = new Date(), nr = new Date(c.next_review), d = Math.ceil((nr - now) / 864e5)
...
function maturityLabel(c) {
  const r = Number(c.repetitions) || 0, iv = Number(c.interval) || 0
  if (r === 0) return 'New'
...
function sm2(option, card) {
  ...
  let ease_factor = Number(card.ease_factor) || 2.5, interval = Number(card.interval) || 0, repetitions = Number(card.repetitions) || 0
  if (quality >= 3) {
...
export default function Anki() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [deckData, cardData] = await Promise.all([apiGet('/decks'), apiGet('/flashcards')])
  ...
  async function createDeck() { if (!deckName.trim()) return; setSavingDeck(true); try { const r = await apiPost('/decks', { name: deckName.trim() }); if (r.error) throw new Error(r.error); setDeckName(''); load() } catch (e) { alert(e.message) } setSavingDeck(false) }
  async function deleteDeck(id) { if (!confirm('Delete deck and all its cards?')) return; try { const r = await apiDel('/decks/' + id); if (r.error) throw new Error(r.error); if (activeDeckId === id) { setActiveDeckId(null); setView('decks') } load() } catch (e) { alert(e.message) } }
  async function addCard() { if (!front.trim() || !back.trim()) return; if (!formDeckId) return alert('Please select or create a deck first.'); setSaving(true); try { const r = await apiPost('/flashcards', { deck_id: formDeckId, front: front.trim(), back: back.trim() }); if (r.error) throw new Error(r.error); setFront(''); setBack(''); setHighYield(false); load() } catch (e) { alert(e.message) } setSaving(false) }
  async function deleteCard(id) { if (!confirm('Delete this card?')) return; try { const r = await apiDel('/flashcards/' + id); if (r.error) throw new Error(r.error); load() } catch (e) { alert(e.message) } }
  ...
  async function submitReview(option) {
    const c = queue[qIdx]; if (!c) return; const u = sm2(option, c)
    try { const r = await apiPut('/flashcards/' + c.id, { ease_factor: u.ease_factor, interval: u.interval, repetitions: u.repetitions, next_review: u.next_review, last_review: u.last_review }); if (r.error) throw new Error(r.error); if (qIdx + 1 < queue.length) { setQIdx(qIdx + 1); setShowAns(false) } else { setView(activeDeckId ? 'browse' : 'decks'); load() } } catch (e) { alert(e.message) }
  }
  function exitReview() { setQueue([]); setQIdx(0); setShowAns(false); setView(activeDeckId ? 'browse' : 'decks'); load() }
  async function handleFile(file) { if (!file) return; setParsing(true); setParseErr(''); setUploadProgress('Parsing file...'); try { const r = await parseFile(file, m => setUploadProgress(m)); if (!r.length) { setParseErr('No cards found.'); setParsing(false); return } setParsed(r); setUploadDeck(activeDeckId || ''); setView('upload') } catch (e) { setParseErr(e.message) } setParsing(false) }
  async function importCards() { if (!parsed.length) return; if (!uploadDeck) return alert('Please select a deck.'); setImporting(true); try { const withDeck = parsed.map(c => ({ front: c.front, back: c.back, deck_id: uploadDeck })); const r = await apiPost('/flashcards', { cards: withDeck }); if (r.error) throw new Error(r.error); setParsed([]); setUploadDeck(''); load(); setView(activeDeckId ? 'browse' : 'decks') } catch (e) { alert(e.message) } setImporting(false) }

  if (loading) return <div className={s.loading}>Loading Anki...</div>
  if (error && !cards.length && !decks.length) return (<div className={s.page}><div className={s.errorBox}><h3>Setup Required</h3><p>Make sure your Turso database has anki_decks and anki_cards tables.</p><p className={s.errorDetail}>{error}</p></div></div>)
  const cur = queue[qIdx]
  const getNrClass = (c) => { const st = statusInfo(c); return st.l === 'Due' ? s.nr_due : st.l === 'New' ? s.nr_soon : s.nr_later }

  return (<div className={s.page}>
    <div className={s.header}>
      <div><h1 className={s.title}>Anki</h1><p className={s.sub}>Spaced repetition Ã¢â‚¬â€ SM-2</p></div>
    ...
    {view === 'decks' && (<>
      ...
      <div className={s.deckGrid}>
        ...
        {decks.map(d => { const dc = cards.filter(c => c.deck_id === d.id), dd = dc.filter(c => isDue(c)); return (
    ...
    {view === 'browse' && (<>
      ...
      <div className={s.cardList}>
        ...
        {vis.map(card => {
          const st = statusInfo(card), r = nextReviewLabel(card)
          return (<div key={card.id} className={s.ankiCard}>
            ...
            <div className={s.ankiStats}><span>EF: {Number(card.ease_factor || 2.5).toFixed(2)}</span><span>Interval: {Number(card.interval) || 0}d</span><span>Reviews: {Number(card.repetitions) || 0}</span><span>Next: {card.next_review ? nextReviewLabel(card).t : '--'}</span></div>
    ...
    {view === 'add' && (<>
      <div className={s.breadcrumb}><button className={s.breadLink} onClick={() => setView(activeDeckId ? 'browse' : 'decks')}>Back</button></div>
      <div className={s.formCard}>
        ...
        <div className={s.uploadSection}>
          ...
          <div className={s.uploadDesc}>Upload .apkg, .csv, .tsv, .txt, or image files to bulk import flashcards. Supports OCR for English + Arabic.</div>
    ...
    {view === 'upload' && parsed.length > 0 && (<>
      <div className={s.breadcrumb}><button className={s.breadLink} onClick={() => { setParsed([]); setView('add') }}>Back</button></div>
      <div className={s.formCard}>
        ...
        <div className={s.previewList}>
          ...
          {parsed.slice(0, 30).map((c, i) => (<div key={i} className={s.previewRow}><span className={s.previewFront}>{c.front}</span><span className={s.previewArrow}>{'\u2192'}</span><span className={s.previewBack}>{c.back}</span></div>))}
    ...
    {view === 'review' && cur && (<>
      ...
      <div className={s.reviewCard}>
        ...
        {!showAns ? (
          ...
          <div className={s.reviewMeta}><span>EF: {Number(cur.ease_factor || 2.5).toFixed(2)}</span><span>Interval: {Number(cur.interval) || 0}d</span><span>Reviews: {Number(cur.repetitions) || 0}</span></div>
