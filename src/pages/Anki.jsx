...
export default function Anki() {
  ...
  async function submitReview(outcome) {
    ...
    const isFirst = intv === 0 // brand new card, never reviewed successfully
    ...
    switch (outcome) {
      case 'again':
        ...
        newIntv = 1   // reset to 1 day
        break
      case 'hard':
        newEf = Math.max(1.3, ef - 0.15)
        newIntv = isFirst ? 3 : Math.max(2, Math.ceil(intv * 1.2))
        break
      case 'good':
        newEf = ef
        newIntv = isFirst ? 6 : Math.max(2, Math.ceil(intv * ef))
        break
      case 'easy':
        newEf = ef + 0.15
        newIntv = isFirst ? 10 : Math.max(2, Math.ceil(intv * ef * 1.3))
        break
      default:
